...
const getLoginSchema = (t: (path: string) => string) => z.object({
  email: z.string().email(t('portal.auth.errors.invalidEmail' as any)),
  password: z.string().min(6, t('portal.auth.errors.weakPassword' as any)),
});
    ...
    <div className="w-full max-w-[400px] space-y-8">
      ...
      <div className="flex flex-col items-center justify-center space-y-4">
        ...
        <div className="text-center">
          <h1 className="text-2xl font-bold tracking-tight text-surface-900 dark:text-white">{t('portal.auth.login.title')}</h1>
          <p className="text-surface-500 dark:text-surface-400 mt-1">{t('portal.auth.login.subtitle')}</p>
        </div>
      </div>
      ...
      <PortalCard className="p-8 shadow-xl">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-sm font-medium text-surface-700 dark:text-surface-300">{t('portal.auth.login.password')}</label>
                <Link
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-white dark:bg-surface-900 px-2 text-surface-500">{t('portal.auth.login.sso')}</span>
            </div>
          </div>

          <PortalButton variant="outline" className="w-full h-11 border-surface-200 dark:border-surface-800" type="button">
        ...
        <p className="text-center text-sm text-surface-500 dark:text-surface-400 mt-6">
          ...
          <Link
            href={redirectPath ? `/portal/signup?redirect=${encodeURIComponent(redirectPath)}` : "/portal/signup/"}
