...
export const ExitIntentModal: React.FC<ExitIntentModalProps> = ({ delay = 0, storageKey = "exitIntentShown" }) => {
	...
	const content = {
		...
		description: isHe ? "נשמח לשמוע על מה שאתם בונים ולראות איך נוכל לעזור." : "We'd love to hear about what you're building and see how we can help.",
	...
	return (
		<AnimatePresence mode="wait">
			{isOpen && (
				<>
					<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[200]" onClick={handleBackdropClick} />
					<motion.div
						...
						<div className="bg-white dark:bg-surface-800 rounded-2xl shadow-2xl overflow-hidden">
							<div className="relative bg-gradient-to-br from-accent-600 to-accent-700 p-8 text-white">
								<button onClick={handleCloseClick} className="absolute top-4 end-4 p-2 rounded-full hover:bg-white/10 transition-colors" aria-label="Close">
									<Icon name="x" size={20} />
								</button>
							...
							<div className="p-6">
								<p className="text-slate-600 dark:text-surface-400 text-center mb-6">{content.description}</p>
