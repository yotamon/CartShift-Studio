...
export const BlogPostContent: React.FC<BlogPostContentProps> = ({ content, relatedPosts, title, date, category, readingTime }) => {
						...
						<aside className="lg:col-span-3 hidden lg:block relative">
							<div className="lg:sticky lg:top-28">
											...
											{headings.map((heading, index) => (
												<button
													...
													<span className="flex items-center gap-2">
														<span className="text-xs text-slate-400 dark:text-surface-500 font-mono group-hover:text-primary-500 transition-colors">{String(index + 1).padStart(2, "0")}</span>
								...
								<div className="p-4 rounded-xl bg-gradient-to-br from-primary-50 to-accent-50 dark:from-primary-900/20 dark:to-accent-900/20 border border-primary-200/50 dark:border-primary-800/30">
									<p className="text-xs font-medium text-slate-600 dark:text-surface-400 mb-3 uppercase tracking-wider">{isHe ? "שתף מאמר" : "Share Article"}</p>
									<div className="flex gap-2">
										<button
											...
											<svg className="w-4 h-4 mx-auto text-slate-500 dark:text-surface-400 group-hover:text-[#1DA1F2] transition-colors" fill="currentColor" viewBox="0 0 24 24">
										...
										<button
											...
											<svg className="w-4 h-4 mx-auto text-slate-500 dark:text-surface-400 group-hover:text-[#0A66C2] transition-colors" fill="currentColor" viewBox="0 0 24 24">
										...
										<button
											...
											<svg className="w-4 h-4 mx-auto text-slate-500 dark:text-surface-400 group-hover:text-primary-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
												<path
													strokeLinecap="round"
							...
							<motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
								...
								<div className="mb-10 p-6 rounded-2xl bg-gradient-to-br from-white via-white to-slate-50 dark:from-surface-800 dark:via-surface-800 dark:to-surface-900 border border-slate-200 dark:border-surface-700 shadow-sm">
									<div className="flex flex-wrap items-center gap-4 text-sm">
										<span className="px-4 py-1.5 rounded-full bg-gradient-to-r from-primary-500 to-primary-600 text-white font-semibold shadow-sm">{category}</span>
										<time dateTime={date} className="flex items-center gap-2 text-slate-600 dark:text-surface-400">
									...
									<div className="lg:hidden flex items-center gap-3 pt-4 mt-4 border-t border-slate-200 dark:border-surface-700">
										<span className="text-sm font-medium text-slate-600 dark:text-surface-400">{isHe ? "שתף:" : "Share:"}</span>
										<div className="flex gap-2">
											<button onClick={shareToTwitter} className="p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-surface-700 transition-colors group" aria-label="Share on Twitter">
												<svg className="w-5 h-5 text-slate-500 dark:text-surface-400 group-hover:text-[#1DA1F2] transition-colors" fill="currentColor" viewBox="0 0 24 24">
													<path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
												</svg>
											</button>
											<button onClick={shareToLinkedIn} className="p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-surface-700 transition-colors group" aria-label="Share on LinkedIn">
												<svg className="w-5 h-5 text-slate-500 dark:text-surface-400 group-hover:text-[#0A66C2] transition-colors" fill="currentColor" viewBox="0 0 24 24">
													<path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
												</svg>
											</button>
											<button onClick={shareToFacebook} className="p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-surface-700 transition-colors group" aria-label="Share on Facebook">
												<svg className="w-5 h-5 text-slate-500 dark:text-surface-400 group-hover:text-[#1877F2] transition-colors" fill="currentColor" viewBox="0 0 24 24">
													<path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
												</svg>
											</button>
											<button onClick={copyLink} className="p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-surface-700 transition-colors group" aria-label="Copy link">
												<svg className="w-5 h-5 text-slate-500 dark:text-surface-400 group-hover:text-primary-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
													<path
														strokeLinecap="round"
														strokeLinejoin="round"
														strokeWidth={2}
								...
								<Card className="overflow-hidden">
									<CardContent className="p-6 sm:p-8 md:p-10 lg:p-12">
										<article ref={articleRef} className="prose-article" dangerouslySetInnerHTML={{ __html: content }} />
									</CardContent>
								</Card>
								...
								<motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }} className="mt-12 relative">
									...
									<div className="relative rounded-2xl p-8 md:p-10 lg:p-12 bg-white dark:bg-surface-900 text-center overflow-hidden">
										...
										<div className="relative z-10 space-y-5">
											...
											<div className="pt-2">
												<Link href="/contact">
													<Button size="lg" variant="secondary" className="text-base md:text-lg px-8 md:px-12 py-4 md:py-5 font-bold shadow-glow-primary group">
