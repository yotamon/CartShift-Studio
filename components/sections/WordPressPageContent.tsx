...
export const WordPressPageContent: React.FC = () => {
    ...
    const services = t("wordpress.services.items") as any[];
    const whyItems = t("wordpress.why.items") as any[];
    const faqData = t("wordpress.faq") as unknown as { title: string; subtitle: string; items: FAQItem[] };
    const faqItems = faqData?.items || [];
    const learnMoreData = t("wordpress.learnMore") as { title: string; excerpt: string; category: string; date: string; href: string };
    const formattedDate = new Date(learnMoreData.date).toLocaleDateString(language === 'he' ? 'he-IL' : 'en-US');

  return (
    <>
      <Section background="default" className="relative overflow-hidden">
        <SectionHeader
          title={t("wordpress.services.title") as string}
          subtitle={t("wordpress.services.subtitle") as string}
        />
      ...
      <Section background="light" className="relative overflow-hidden">
        <div className="max-w-4xl mx-auto">
          ...
          >
            {t("wordpress.why.title") as string} <span className="gradient-text">{t("wordpress.why.titleSpan") as string}</span>
          </motion.h2>
          <div className="space-y-6">
            {whyItems.map((item, index) => (
              ...
              >
                <strong className="text-slate-900 dark:text-white">{item.strong}</strong> {item.text}
              </motion.p>
            ))}
          </div>
          <motion.div
          ...
          >
            <Card hover glow className="h-full group relative overflow-hidden">
              <CardHeader>
                ...
                <CardTitle>
                  <Link href={learnMoreData.href} className="text-slate-900 dark:text-white hover:text-accent-600 dark:hover:text-primary-400 transition-colors">
                    {learnMoreData.title}
                  </Link>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="mb-6 text-base md:text-lg text-slate-600 dark:text-surface-300 leading-relaxed">{learnMoreData.excerpt}</p>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-slate-900 dark:text-white font-display mb-8 leading-tight tracking-tight">
              {t("wordpress.cta.title") as string} <span className="gradient-text">{t("wordpress.cta.titleSpan") as string}</span>
            </h2>
            <p className="text-base md:text-lg text-slate-600 dark:text-surface-300 mb-8 max-w-2xl mx-auto leading-relaxed">
              {t("wordpress.cta.description") as string}
            </p>
            <Link href="/contact">
              <Button size="lg" className="group">
                <span className="relative z-10 flex items-center gap-2">
                  {t("wordpress.cta.button") as string}
