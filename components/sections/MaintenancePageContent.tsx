...
const PlanCard: React.FC<PlanCardProps> = ({
  ...
  >
    ...
    <div className={`h-full rounded-2xl p-6 lg:p-8 flex flex-col ${
      popular
        ? "bg-accent-600 text-white ring-2 ring-accent-500 ring-offset-2 ring-offset-white dark:ring-offset-surface-950"
        : "bg-white dark:bg-surface-800 border border-slate-200 dark:border-surface-700"
    }`}>
      <div className="mb-6">
        <h3 className={`text-xl font-bold mb-2 ${popular ? "text-white" : "text-slate-900 dark:text-white"}`}>
          {name}
        </h3>
        <p className={`text-sm ${popular ? "text-white/80" : "text-slate-600 dark:text-surface-400"}`}>
          {description}
        </p>
      </div>
      <div className="mb-6">
        <div className={`text-3xl font-bold ${popular ? "text-white" : "text-slate-900 dark:text-white"}`}>
          {price}
        </div>
      </div>
      <ul className="space-y-3 mb-8 flex-grow">
        {features.map((feature, i) => (
          <li key={i} className="flex items-start gap-3">
            ...
            />
            <span className={`text-sm ${popular ? "text-white/90" : "text-slate-600 dark:text-surface-400"}`}>
              {feature}
            </span>
          </li>
        ))}
...
export const MaintenancePageContent: React.FC = () => {
  ...
  const maintenance = t("maintenance") as {
    ...
    plans: {
      essential: { name: string; price: string; description: string; features: string[]; cta: string };
      growth: { name: string; price: string; popular: boolean; description: string; features: string[]; cta: string };
      premium: { name: string; price: string; description: string; features: string[]; cta: string };
    };
    coverage: {
      title: string;
                ...
                <CardHeader>
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-xl bg-accent-100 dark:bg-accent-900/30 flex items-center justify-center">
                      <Icon name={section.icon} size={20} className="text-accent-600 dark:text-accent-400" />
                    </div>
                  ...
                  <ul className="space-y-2">
                    {section.items.map((item, j) => (
                      <li key={j} className="flex items-start gap-2 text-sm">
                        <Icon name="check" size={16} className="mt-0.5 text-accent-600 dark:text-accent-400 flex-shrink-0" />
                        <span className="text-slate-600 dark:text-surface-400">{item}</span>
                      </li>
                    ))}
                  </ul>
                </CardContent>
              ...
              <CardContent>
                <ul className="space-y-3">
                  {maintenance.terms.items.map((item, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <Icon name="info" size={18} className="mt-0.5 text-slate-400 dark:text-surface-500 flex-shrink-0" />
                      <span className="text-slate-600 dark:text-surface-400">{item}</span>
                    </li>
                  ))}
                </ul>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-slate-900 dark:text-white font-display mb-8 leading-tight tracking-tight">
              {maintenance.cta.title} <span className="gradient-text">{maintenance.cta.titleSpan}</span>
            </h2>
